...
use VkPhotosAndAlbums\Models\User as ChildUser;
use VkPhotosAndAlbums\Models\UserQuery as ChildUserQuery;
use VkPhotosAndAlbums\Models\Map\UserTableMap;
 ...
 * @method     ChildUserQuery orderByLastSync($order = Criteria::ASC) Order by the last_sync column
 *
 ...
 * @method     ChildUserQuery groupByLastSync() Group by the last_sync column
 *
 ...
 * @method     ChildUser findOneByLastSync(string $last_sync) Return the first ChildUser filtered by the last_sync column *
 ...
 * @method     ChildUser requireOneByLastSync(string $last_sync) Return the first ChildUser filtered by the last_sync column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 *
 ...
 * @method     ChildUser[]|ObjectCollection findByLastSync(string $last_sync) Return ChildUser objects filtered by the last_sync column
...
abstract class UserQuery extends ModelCriteria
{
    ...
    /**
     * Initializes internal state of \VkPhotosAndAlbums\Models\Base\UserQuery object.
     *
     * @param     string $dbName The database name
     * @param     string $modelName The phpName of a model, e.g. 'Book'
     * @param     string $modelAlias The alias for the model in this query, e.g. 'b'
     */
    public function __construct($dbName = 'vk_photos_and_albums', $modelName = '\\VkPhotosAndAlbums\\Models\\User', $modelAlias = null)
    {
    ...
    {
        $sql = 'SELECT id, first_name, last_name, last_sync FROM user WHERE id = :p0';
        try {
    ...
    public function filterByLastSync($lastSync = null, $comparison = null)
    {
        if (is_array($lastSync)) {
            ...
            if (isset($lastSync['min'])) {
                $this->addUsingAlias(UserTableMap::COL_LAST_SYNC, $lastSync['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($lastSync['max'])) {
                $this->addUsingAlias(UserTableMap::COL_LAST_SYNC, $lastSync['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(UserTableMap::COL_LAST_SYNC, $lastSync, $comparison);
    }
